...
namespace ZI___Zadatak_1_od_3.PlayFairCipher
{
    class PlayfairCipher
    {
		...
		private static string AdjustOutput(string input, string output, bool ench)
		{
			...
			if (input.Length % 2 != 0 && ench == true)
			{
				int j = input.Length;
				retVal[j] = char.ToLower(retVal[j]);				
			}
		...
		private static string Cipher(string input, string key, bool encipher)
		{
			...
			retVal = AdjustOutput(input, retVal, encipher);
